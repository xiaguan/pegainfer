...
/// K3 vendored decode kernels (cubin/k3/): external serving kernels bound by
/// the capsule loader in csrc/k3/k3_capsule.cu. Provenance in cubin/k3/README.
const K3_CAPSULE_CUBINS: &[(&str, &str)] = &[
    ("kK3CapsuleTopk", "single_group_topk_e512t22_sm103.cubin"),
    ("kK3CapsuleKdaDecode", "kda_decode_fusion_h96_sm103.cubin"),
];

fn generate_k3_capsule_cubins(root: &Path, out_dir: &Path) {
    let cubin_dir = root.join("cubin/k3");
    let mut generated = String::from(
        "// Generated by pegainfer-kernels/build.rs from the checked-in K3 capsule cubins.\n\
         #pragma once\n\n",
    );
    for &(symbol, file_name) in K3_CAPSULE_CUBINS {
        let path = cubin_dir.join(file_name);
        println!("cargo:rerun-if-changed={}", path.display());
        let bytes = fs::read(&path).unwrap_or_else(|err| {
            panic!("failed to read K3 capsule cubin {}: {err}", path.display())
        });
        writeln!(&mut generated, "static const unsigned char {symbol}[] = {{")
            .expect("write cubin array declaration");
        for chunk in bytes.chunks(16) {
    ...
    let output = out_dir.join("k3_capsule_cubins.inc");
    fs::write(&output, generated).unwrap_or_else(|err| {
        panic!(
            "failed to write generated K3 capsule cubin header {}: {err}",
            output.display()
        )
    });
}
...
fn main() {
    ...
    if k3_enabled {
        generate_k3_capsule_cubins(&crate_root(), &out_dir);
    }
    println!(
    ...
    for cu_file in &cu_files {
        ...
        if stem == "k3_capsule" {
            // Includes the generated k3_capsule_cubins.inc from OUT_DIR.
            nvcc_args.extend(["-I".to_string(), out_dir.to_string_lossy().to_string()]);
        }
