...
| `deepseek_v4.compressor.prefill` | `runtime/compressor.rs` | `deepseek_compressor_nonoverlap_prefill_cuda`, overlap candidate: `deepseek_compressor_overlap_prefill_projected_cuda` | `csrc/deepseek_v4/deepseek_compressor.cu` | CUDA + cuBLAS projection candidate | Non-overlap remains on the CUDA weighted prefill path. This decision branch routes overlap prefill through BF16 cuBLAS projections plus a route-combine kernel to expose the performance/quality tradeoff; `deepseek_compressor_overlap_prefill_cuda` remains the strict-hash reference path. |
