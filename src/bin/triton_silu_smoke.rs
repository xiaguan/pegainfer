...
fn silu_mul_cpu_reference(gate: &[bf16], up: &[bf16]) -> Vec<bf16> {
    gate.iter()
        .zip(up.iter())
        .map(|(gate, up)| {
            let gate = gate.to_f32();
            let silu = gate / (1.0 + (-gate).exp());
            bf16::from_f32(silu * up.to_f32())
        })
        .collect()
}
...
fn main() -> Result<()> {
    ...
    let expected = silu_mul_cpu_reference(&gate_host, &up_host);
    let triton_out = ops::silu_mul_batch(&ctx, &gate, &up)?;
    let triton_host = dtoh(&ctx, &triton_out)?;
    let diff = max_abs_diff(&expected, &triton_host);
