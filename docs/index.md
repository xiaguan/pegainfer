...
| `projects/virtual-workspace-top-level-crates.md` | Root is now a virtual workspace; product code lives in `pegainfer-server/`, internal packages live at top-level `pegainfer-*` paths. Metadata/fmt/check/clippy/test-target compile, refreshed Qwen3 e2e, Qwen3.5 e2e, and bench smoke pass. |
...
| `projects/qwen35-model-crate.md` | Qwen3.5 model crate split implemented: `pegainfer-qwen35-4b` owns Qwen3.5 model/scheduler/recurrent ops/tests/benches; root loads it through `EngineHandle`. Build/check/clippy, root bench smoke, Qwen3.5 e2e, and scheduler e2e pass |
