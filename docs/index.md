...
| `projects/deepseek-moe-tilelang-review.md` | Active: official TileKernels MoE maps routing/layout, not a drop-in expert MLP; persistent DeepSeek rank workers cut 1x32 steady TPOT to `94.84ms/token`; remaining nsys/Amdahl bucket is rank arrival skew from uneven MoE FP4 expert work before `107` f32 collectives/token, not raw NCCL transfer. |
