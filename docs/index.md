...
| `projects/deepseek-moe-tilelang-review.md` | Active: official TileKernels MoE maps routing/layout, not a drop-in expert MLP; persistent DeepSeek rank workers cut 1x32 steady TPOT to `94.84ms/token`; NVTX+CUDA trace proves the remaining nsys/Amdahl bucket is rank arrival skew before `107` f32 collectives/token, not raw NCCL transfer. |
