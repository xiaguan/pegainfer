...
- **Follow-ups**:
  - Rework `pegainfer-qwen3-4b/src/batch_decode.rs` and `pegainfer-qwen35-4b/src/batch_decode.rs` so batch token selection stays batched instead of iterating row by row.
